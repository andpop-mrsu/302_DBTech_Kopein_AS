...
try {
    $stmt = $pdo->prepare("SELECT * FROM completed_services WHERE master_id = ? ORDER BY service_date DESC");
    $stmt->execute([$master_id]);
    $works = $stmt->fetchAll();
} catch (PDOException $e) {
            ...
            <table>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><?= htmlspecialchars(date('d.m.Y', strtotime($work['service_date']))) ?></td>
