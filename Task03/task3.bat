...
echo "--------------------------------------------------"
sqlite3 movies_rating.db -box -echo "SELECT u.name as expert, m.title, m.year, r.rating, date(r.timestamp, 'unixepoch') as rating_date FROM ratings r JOIN users u ON r.user_id = u.id JOIN movies m ON r.movie_id = m.id ORDER BY u.name, m.title, r.rating LIMIT 50;"
echo " "
...
echo "--------------------------------------------------"
sqlite3 movies_rating.db -box -echo "SELECT m.title, m.year, COUNT(*) as high_ratings_count FROM movies m JOIN ratings r ON m.id = r.movie_id JOIN users u ON r.user_id = u.id WHERE m.year > 2005 AND m.genres LIKE '%Drama%' AND u.gender = 'female' AND r.rating >= 4.5 GROUP BY m.id, m.title, m.year ORDER BY m.year, m.title;"
echo " "
...
echo "--------------------------------------------------"
sqlite3 movies_rating.db -box -echo "SELECT substr(register_date, 1, 4) as year, COUNT(*) as user_count FROM users GROUP BY year ORDER BY user_count DESC;"
echo " "

echo "Год с наибольшим количеством регистраций:"
sqlite3 movies_rating.db -box -echo "SELECT substr(register_date, 1, 4) as year, COUNT(*) as user_count FROM users GROUP BY year ORDER BY user_count DESC LIMIT 1;"
echo " "

echo "Год с наименьшим количеством регистраций:"
sqlite3 movies_rating.db -box -echo "SELECT substr(register_date, 1, 4) as year, COUNT(*) as user_count FROM users GROUP BY year ORDER BY user_count ASC LIMIT 1;"
